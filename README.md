...
Developer execution telemetry is sanitized, bounded, and batch-persisted outside provider-call latency; durable user-visible activity, including Deep Research activity, remains separate. Ask RAVEN is persistent profile-grounded Chat: factual company answers require cited evidence, while greetings and product guidance can respond naturally. Web Search is an optional, conversation-persisted capability; when enabled, the agent may acquire bounded web evidence and persists the evidence snapshot behind every web citation. Deep Research remains a separate, explicit workflow.
