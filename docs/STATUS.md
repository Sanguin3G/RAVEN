...
Current branch: **main** (Day 7 integrated)
...
dotnet test Raven.sln --no-restore    293 passed
npm test -- --run                     54 passed across 12 files (deterministic chunks)
npm run build                         passed
```

Microsoft Edge/Playwright runs through the explicit `msedge` project/channel verified deterministic resolved and family identity flows, exactly one discovery phase, canonical execution-detail rendering, Ask RAVEN greeting/factual citation/research-guidance behavior, the truthful capability menu and Investigations handoff, all Company workspace tabs, and 390×844 mobile layout. The API smoke test applied all migrations, including Chat, to a clean temporary SQLite database and returned 200 for `/health` and OpenAPI. No paid provider call was made by the Day-7 automated suite.
...
- Web-enabled normal Chat, actual Search-the-web capability, and rendering or launching Deep Research directly inside a conversation.
