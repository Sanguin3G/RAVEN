...
The persisted `ChatModel` role controls both Ask RAVEN answers and the Gemini rewrite that prepares an editable Managed Research question. It is independent of `ProfileModel` and `DeepResearchModel`; migration copies each workspace's former effective chat choice from `ProfileModel` once, then the roles can change independently. A running chat turn reads the selected role once at its start, while brief preview reads it when the preview is requested.
...
Native Research keeps `ResearchCompanyService` as the workflow facade while discovery/ranking and evidence acquisition are isolated feature responsibilities. Managed AI Research is a durable asynchronous workflow: an Exa Agent run is queued, polled by an isolated background worker, normalized into a company Investigation, and surfaced through lightweight client polling/notification. It never mutates the accepted profile. In Ask RAVEN, a bounded Gemini structured rewrite turns the user's request and server-owned company context into one editable research question before any managed job or conversation is created. The question preserves all requested topics and the user's language; it must not create factual claims or research results, and it never reads existing Investigations. Confirming the question validates its company-context revision and sends it with server-owned company context and evidence rules to the provider; a failed rewrite blocks Start rather than echoing the original request. The separate bounded Gemini/MAF Deep Research API remains available for explicit tool-assisted runs. Ask RAVEN requires an accepted profile and can attach up to five completed Managed Research Investigations to a conversation. Attached summaries, claims, source leads, and uncertainties are bounded, unaccepted chat context with separate Investigation citations, never accepted profile truth or input to a new research job. A Managed Research job started from chat persists the approved question; on completion its worker attaches the Investigation and saves one answer grounded in that result. Standalone Managed Research jobs do not enter chat automatically.

Ask RAVEN Web Search is a conversation-persisted permission, not a forced action. Normal Chat still requires an accepted profile and reads profile evidence first. When enabled and the question needs missing or freshness-sensitive company facts, the Chat agent may use the normal `ISearchProvider` and `ICrawlerProvider` routes. Web candidates are turn-scoped; cited pages are persisted as `ChatWebEvidenceSnapshot` rows and validated independently from accepted `ProfileEvidence`. Server-sent events expose bounded progress without persisting hidden reasoning or raw provider payloads.
