...
Backend ownership is organized by Companies, Research, Investigations, Profiles, Chat, Monitoring, and Settings. Research obtains information; Investigations preserve durable research material; Briefings synthesize selected Investigations into thematic views; Profiles own accepted standardized truth; Chat is a conversational surface over permitted context; Monitoring coordinates review-ready refreshes. Frontend company-workspace, company-research, and Ask RAVEN features follow the same ownership direction.
...
The persisted `ChatModel` role controls both Ask RAVEN answers and the Gemini rewrite that prepares an editable Managed Research question. It is independent of `ProfileModel` and `DeepResearchModel`; migration copies each workspace's former effective chat choice from `ProfileModel` once, then the roles can change independently. A running chat turn reads the selected role once at its start, while brief preview reads it when the preview is requested. Briefing generation reads the persisted `DeepResearchModel` at the start of each synthesis request; it does not rely on process-local model state that can diverge after an API restart.
...
Gemini configuration establishes startup defaults. Persistent research settings keep independent approved roles for identity grounding, profile generation, Ask RAVEN plus managed-research question rewriting, and native synthesis/Deep Research. The Settings page limits each role to RAVEN's compatibility-tested model catalog and reports project availability without exposing credentials. Managed Research through Exa Agent keeps its provider-owned model boundary.

## Workspace composition

The React application uses a fixed, collapsible desktop sidebar and a mobile drawer, contextual top bar, System Status route, reusable source cards/icons, and Settings. Company workspace tabs are Overview, Sources, Investigations, Briefings, Changes, and Monitoring. A desktop Ask RAVEN dock reflows the dossier rather than overlaying it; on mobile it becomes a drawer. The frontend may render company context and composer modes, but Hung owns the Ask RAVEN backend/conversation contract. Source icons use safe domain favicon resolution with provider-aware and Phosphor fallbacks.

`CompanyLifecycleService` is the mutation boundary for archive, restore, permanent deletion, and user-confirmed merge; `CompanyDeletionService` and `CompanyMergeService` own the cohesive deletion and merge workflows behind that facade. Merge is transactional and retains compatible research, source, profile, provenance, monitoring, Deep Research, and saved-investigation relationships. It rewrites moved serialized profile/candidate identity references and reorders the combined immutable profile history by confirmation time, so the current accepted profile remains a valid, hydrated snapshot rather than being masked by a sparse retained Company row. Missing stable Company identity values are filled from the merged record; existing canonical values remain authoritative. `CompanyWorkspaceReviewService` is read-only: it evaluates deterministic health and duplicate groups, and its optional AI seam can only return recommendations. A separate `WorkspaceResearchReviewState` records only user acknowledgement watermarks for the compact research queue; its acknowledge and smart-cleanup endpoints preserve all runs, investigations, sources, and evidence.
...
Native Research keeps `ResearchCompanyService` as the workflow facade while discovery/ranking and evidence acquisition are isolated feature responsibilities. Managed AI Research is a durable asynchronous workflow: an Exa Agent run is queued, polled by an isolated background worker, normalized into a company Investigation, and surfaced through lightweight client polling/notification. It never mutates the accepted profile. In Ask RAVEN, a bounded Gemini structured rewrite turns the user's request and server-owned company context into one editable research question before any managed job or conversation is created. The question preserves all requested topics and the user's language; it must not create factual claims or research results, and it never reads existing Investigations. Confirming the question validates its company-context revision and sends it with server-owned company context and evidence rules to the provider; a failed rewrite blocks Start rather than echoing the original request. The separate bounded Gemini/MAF Deep Research API remains available for explicit tool-assisted runs. Ask RAVEN requires an accepted profile and can attach at most five combined completed Investigations and immutable Briefing versions to a conversation. Investigation attachments contribute bounded summaries, claims, source leads, and uncertainties. Briefing attachments pin an exact version and contribute only synthesized sections plus source-Investigation metadata and uncertainties; raw material, claims, and source leads are deliberately excluded. Both remain unaccepted chat context with distinct citations, never accepted Profile truth or input to a new research job. A Briefing citation deep-links to its pinned historical version without replacing it with the latest version. A Managed Research job started from chat persists the approved question; on completion its worker attaches the Investigation and saves one answer grounded in that result. Standalone Managed Research jobs do not enter chat automatically.

Ask RAVEN Web Search is a conversation-persisted permission. Normal Chat still requires an accepted profile and reads profile evidence first. When enabled, a normal factual company question runs through a bounded research orchestrator rather than keyword-triggered tool choice: Gemini returns structured planning and coverage decisions, while deterministic application policy owns company identity enrichment, budgets, stopping, URL eligibility, and citation validation. The default budget is three research rounds, three searches, five crawls, four model calls including final synthesis, and a 150-second turn deadline with a reserved final-answer window. Search candidates and question-aware Markdown excerpts accumulate across research rounds, but final synthesis receives only acquired evidence; its dynamic schema enumerates the citation IDs valid for that turn. One bounded repair is allowed when model-call budget remains, otherwise invalid final citations degrade to `insufficient_evidence` while preserving safe tool results instead of surfacing a provider-format error. Ranking considers company identity, question relevance, requested-year coverage, authority, crawlability, provider rank, and source diversity. Managed Research completion that must cite its attached Investigation does not start a redundant Web loop.

Turn-local Web, Investigation, and Briefing material is split into bounded heading/paragraph-aware chunks and selected with lexical, year, and metadata signals. Persistent `SourceChunk` rows, embeddings, and vector retrieval remain future work. Web candidates stay turn-scoped; successfully read pages are persisted as `ChatWebEvidenceSnapshot` rows, cited excerpts are retained on citations, and they remain independent from accepted `ProfileEvidence`. Server-sent events expose bounded progress without persisting prompts, hidden reasoning, or raw provider payloads.

An Investigation is the durable research workspace, not a provider-result type. Native Research, Managed AI/Deep Research, and External AI-assisted research share the same review surface. Origin records provenance; explicit Purpose (`GeneralResearch` or `ProfileImprovement`) controls workflow; multi-valued Topics organize material; effective status is Running, Ready, Done, or Failed. New purpose is assigned by the creating workflow, never title keywords. Historical ambiguous saved material defaults to General Research. `InvestigationReviewState` persists Done/Reopen watermarks and an applied Profile version when applicable; newer material can become Ready again. A usable accepted Profile gates the Profile Improvement action, not General Research or Briefings. Raw material is preserved; `InvestigationOrganizationRevision` is a replaceable, versioned analysis of one Investigation and cannot mutate accepted profile truth. External analysis is a durable queued task: deterministic Markdown parsing produces reviewable claims, source leads, and uncertainties, while the original response remains available when analysis fails. Deep/External results can enter the existing server-owned targeted profile-patch workflow as imported material; the user still reviews and confirms an immutable profile version, and no second crawl is launched for provider citations.

`ResearchBriefing` is company-scoped and holds title, controlled template, and objective. Each `ResearchBriefingVersion` is an immutable generated snapshot with structured sections, selected Investigation source snapshots, Generated time, and ResearchThrough derived from selected material. Creation and Update synthesize only persisted selected research through the existing AI abstraction: Update retains previous source snapshots and adds human-selected material, never launching Search, Crawl, or a Profile mutation. Newer-research suggestions use company, date, and topic overlap. Research latest/gaps hands off to existing Deep Research or External Assist; the resulting Investigation must be selected for a later Briefing version. Briefings are reusable research views, not accepted Profile truth.
