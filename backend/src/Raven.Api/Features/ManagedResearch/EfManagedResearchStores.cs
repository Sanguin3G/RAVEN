...
public sealed class EfManagedResearchJobStore(RavenDbContext db) : IManagedResearchJobStore
{
    ...
    public Task<IReadOnlyList<ManagedResearchJob>> ListActiveAsync(CancellationToken ct = default) => db.ManagedResearchJobs.Where(x => x.Status == ManagedResearchJobStatus.Queued || x.Status == ManagedResearchJobStatus.Researching).OrderBy(x => x.CreatedAt).ToListAsync(ct).ContinueWith<IReadOnlyList<ManagedResearchJob>>(x => x.Result, ct);
    public Task<IReadOnlyList<ManagedResearchJob>> ListForCompanyAsync(Guid companyId, CancellationToken ct = default) => db.ManagedResearchJobs.Where(x => x.CompanyId == companyId).OrderByDescending(x => x.CreatedAt).ToListAsync(ct).ContinueWith<IReadOnlyList<ManagedResearchJob>>(x => x.Result, ct);
}
...
public sealed class EfManagedResearchInvestigationStore(RavenDbContext db) : IManagedResearchInvestigationStore
{
    ...
    public Task<IReadOnlyList<ManagedResearchInvestigation>> ListForCompanyAsync(Guid companyId, CancellationToken ct = default) => db.ManagedResearchInvestigations.AsNoTracking().Where(x => x.CompanyId == companyId).OrderByDescending(x => x.CompletedAt).ToListAsync(ct).ContinueWith<IReadOnlyList<ManagedResearchInvestigation>>(x => x.Result, ct);
}
