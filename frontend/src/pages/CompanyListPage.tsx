...
import {
  archiveCompany,
  acknowledgeWorkspaceResearch,
  cleanupWorkspaceResearch,
  confirmCompanyMerge,
  deleteCompany,
  getWorkspaceReview,
  previewCompanyMerge,
...
function ResearchReviewGroup({ title, items, icon: Icon, tone, onOpen, onMarkReviewed, busy, showHeading = true }: { title: string; items: WorkspaceResearchReviewItem[]; icon: typeof CheckCircle; tone: "ready" | "issue"; onOpen: (item: WorkspaceResearchReviewItem) => void; onMarkReviewed: (item: WorkspaceResearchReviewItem) => void; busy: boolean; showHeading?: boolean }) {
  if (items.length === 0) return null;
  return <section className={`workspace-review-section workspace-review-section--research workspace-review-section--${tone}`}>{showHeading ? <div className="workspace-review-section__heading"><div><h3><Icon size={18} weight="fill" aria-hidden="true" /> {title}</h3><span>{items.length} group{items.length === 1 ? "" : "s"}</span></div></div> : null}<div className="workspace-review-research-list">{items.slice(0, 8).map((item) => <article className="workspace-review-research-item" key={item.reviewKey ?? `${item.method}-${item.itemId}`}><div><strong>{item.companyName}</strong><small>{item.method} · {item.title}{(item.occurrenceCount ?? 1) > 1 ? ` · ${item.occurrenceCount} results grouped` : ""}</small>{tone === "issue" && item.detail ? <details className="workspace-review-research-detail"><summary>Show issue detail</summary><p>{item.detail}</p></details> : null}{item.groupingNote ? <span className="workspace-review-grouping-note">{item.groupingNote}</span> : null}</div><div className="workspace-review-research-actions"><button className="button button--secondary" type="button" onClick={() => onOpen(item)}>Review</button><button className="button button--quiet" type="button" disabled={busy} onClick={() => onMarkReviewed(item)}>Mark done</button></div></article>)}</div>{items.length > 8 ? <p className="workspace-review-more">Showing 8 of {items.length} groups. Older results remain preserved in the company investigation.</p> : null}</section>;
}
...
export function CompanyListPage() {
  ...
  async function loadWorkspace(includeArchived = showArchived) {
    ...
    try {
      ...
      const migratable = [...(workspace.researchReady ?? []), ...(workspace.researchIssues ?? [])]
        ...
        .map((item) => ({ reviewKey: item.reviewKey!, acknowledgedThrough: item.updatedAt }));
  ...
  async function acknowledgeItems(items: WorkspaceResearchReviewItem[]) {
    ...
    try {
      await acknowledgeWorkspaceResearch(payload);
      dismissResearchActivities((activity) => items.some((item) => activity.jobId === item.itemId));
      await loadWorkspace(showArchived);
      return true;
    } catch (reason) {
      setCleanupError(getApiErrorMessage(reason, "Could not update the research review queue."));
      return false;
    } finally {
  ...
  async function openResearchReview(item: WorkspaceResearchReviewItem) {
  ...
  async function markAllResearchReviewed() {
    if (await acknowledgeItems([...researchReady, ...researchIssues])) setBulkReviewDialogOpen(false);
  }
  ...
  async function runSmartCleanup() {
    ...
    setCleanupError(null);
    try {
      await cleanupWorkspaceResearch(showArchived);
      setCleanupDialogOpen(false);
      await loadWorkspace(showArchived);
    } catch (reason) {
      setCleanupError(getApiErrorMessage(reason, "Could not clean up redundant research issues."));
    } finally {
      setReviewActionBusy(false);
    }
  };
  ...
  return <div className="page-stack">
    ...
    {reviewOpen ? <Panel className="workspace-review-panel" title="Workspace review" eyebrow="ATTENTION QUEUE"><div className="workspace-review-panel__header"><p>One compact queue for research results, profile health, and identity cleanup.</p><div className="workspace-review-panel__header-actions"><button className="button button--quiet" type="button" onClick={() => setReviewOpen(false)}>Close</button></div></div>{reviewLoading ? <div className="empty-state"><strong>Reviewing workspace…</strong><p>Checking identity, coverage, and recency.</p></div> : null}{reviewError ? <div className="form-error" role="alert">{reviewError}<button className="button button--quiet" type="button" onClick={() => void loadWorkspace(showArchived)}>Retry</button></div> : null}{cleanupError ? <div className="form-error" role="alert">{cleanupError}</div> : null}{!reviewLoading && review ? <><div className="workspace-review-overview" aria-label="Workspace review summary"><ReviewMetric label="Research ready" count={researchReady.length} detail="Open findings" tone="ready" /><ReviewMetric label="Profile attention" count={attentionRecommendations.length} detail="Health and recency" tone="attention" /><ReviewMetric label="Duplicates" count={review.duplicateGroups.length} detail="Identity cleanup" tone="duplicate" /><ReviewMetric label="Research issues" count={researchIssues.length} detail="Grouped failures" tone="issue" /></div><div className="workspace-review-count"><span><strong>{workspaceAttentionCount + researchReady.length + researchIssues.length}</strong> workspace item{workspaceAttentionCount + researchReady.length + researchIssues.length === 1 ? "" : "s"}</span><div className="workspace-review-bulk-actions"><details className="workspace-review-more-actions"><summary className="button button--quiet">More</summary><div><button type="button" disabled={reviewActionBusy || researchReady.length + researchIssues.length === 0} onClick={() => setBulkReviewDialogOpen(true)}>Mark all research reviewed</button></div></details><button className="button button--secondary" type="button" disabled={reviewActionBusy || !(review.researchCleanupCandidates?.length)} onClick={() => setCleanupDialogOpen(true)}>Smart clean-up{review.researchCleanupCandidates?.length ? ` (${review.researchCleanupCandidates.length})` : ""}</button></div><small>Marking an item reviewed only clears its current notification; research history and evidence remain preserved.</small></div>{researchReady.length === 0 && attentionRecommendations.length === 0 && review.duplicateGroups.length === 0 && researchIssues.length === 0 ? <div className="empty-state"><strong>No records need attention.</strong><p>New review items appear here when research completes or profile checks detect a gap.</p></div> : null}{researchReady.length > 0 ? <details className="workspace-review-category" open><summary><span>Research ready for review</span><strong>{researchReady.length}</strong></summary><ResearchReviewGroup title="Research ready for review" items={researchReady} icon={CheckCircle} tone="ready" onOpen={openResearchReview} onMarkReviewed={item => void acknowledgeItems([item])} busy={reviewActionBusy} showHeading={false} /></details> : null}{attentionRecommendations.length > 0 ? <details className="workspace-review-category" open={attentionRecommendations.length <= 2}><summary><span>Companies needing attention</span><strong>{attentionRecommendations.length}</strong></summary><section className="workspace-review-section"><div className="workspace-review-grid">{attentionRecommendations.map((recommendation) => <article className="workspace-review-card" key={`${recommendation.kind}-${recommendation.companyId}-${recommendation.title}`}><div className="workspace-review-card__heading"><Sparkle size={18} aria-hidden="true" /><span><strong>{companies.find((company) => company.id === recommendation.companyId)?.name ?? "Company record"}</strong><small>{recommendation.title}</small></span>{recommendation.healthStatus ? <span className={statusClass(recommendation.healthStatus)}>{statusLabel(recommendation.healthStatus)}</span> : null}</div><p>{recommendation.summary}</p><div className="workspace-review-card__actions"><button className="button button--secondary" type="button" onClick={() => reviewRecommendation(recommendation)}>{recommendation.kind === "SparseProfile" ? "Research now" : recommendation.kind === "Stale" ? "Refresh" : "Open record"}</button>{recommendation.kind === "SparseProfile" ? <button className="button button--quiet" type="button" onClick={() => { const company = companies.find((item) => item.id === recommendation.companyId); if (company) void updateArchive(company, false); }}>Archive</button> : null}</div></article>)}</div></section></details> : null}{review.duplicateGroups.length > 0 ? <details className="workspace-review-category" open={review.duplicateGroups.length <= 1}><summary><span>Possible duplicates</span><strong>{review.duplicateGroups.length}</strong></summary><section className="workspace-review-section"><div className="workspace-review-grid">{review.duplicateGroups.map((group) => <article className="workspace-review-card" key={group.groupId}><div className="workspace-review-card__heading"><GitMerge size={18} aria-hidden="true" /><span><strong>{group.members.map((member) => member.name).join(" / ")}</strong><small>{statusLabel(group.strongestMatch)}</small></span></div><p>{group.rationale}</p><button className="button button--secondary" type="button" onClick={() => reviewGroup(group)}>Review merge</button></article>)}</div></section></details> : null}{researchIssues.length > 0 ? <details className="workspace-review-category workspace-review-category--issues"><summary><span>Research issues</span><strong>{researchIssues.length}</strong></summary><ResearchReviewGroup title="Research issues" items={researchIssues} icon={WarningCircle} tone="issue" onOpen={openResearchReview} onMarkReviewed={item => void acknowledgeItems([item])} busy={reviewActionBusy} showHeading={false} /></details> : null}</> : null}</Panel> : null}
    ...
    <WorkspaceDialog open={cleanupDialogOpen} title="Smart clean-up research issues?" labelledBy="cleanup-dialog-title" onClose={() => { if (!reviewActionBusy) setCleanupDialogOpen(false); }}><p className="workspace-dialog__intro">RAVEN found {review?.researchCleanupCandidates?.length ?? 0} repeated issue group{(review?.researchCleanupCandidates?.length ?? 0) === 1 ? "" : "s"}. This will mark only those redundant groups as done; research history and evidence remain intact.</p>{review?.researchCleanupCandidates?.length ? <ul className="review-reasons">{review.researchCleanupCandidates.slice(0, 8).map((candidate) => <li key={candidate.reviewKey}><strong>{candidate.companyName}</strong> · {candidate.title} ({candidate.occurrenceCount} results)</li>)}</ul> : null}<div className="modal-actions"><button className="button button--secondary" type="button" disabled={reviewActionBusy} onClick={() => setCleanupDialogOpen(false)}>Cancel</button><button className="button" type="button" disabled={reviewActionBusy || !(review?.researchCleanupCandidates?.length)} onClick={() => void runSmartCleanup()}>{reviewActionBusy ? "Cleaning…" : "Clean up redundant issues"}</button></div></WorkspaceDialog>
    <WorkspaceDialog open={bulkReviewDialogOpen} title="Mark all research reviewed?" labelledBy="bulk-review-dialog-title" onClose={() => { if (!reviewActionBusy) setBulkReviewDialogOpen(false); }}><p className="workspace-dialog__intro">This will clear {researchReady.length + researchIssues.length} current research notification{researchReady.length + researchIssues.length === 1 ? "" : "s"}. The research, Investigations, and evidence are preserved. Newer material can appear in the review queue later.</p><div className="modal-actions"><button className="button button--secondary" type="button" disabled={reviewActionBusy} onClick={() => setBulkReviewDialogOpen(false)}>Cancel</button><button className="button" type="button" disabled={reviewActionBusy || researchReady.length + researchIssues.length === 0} onClick={() => void markAllResearchReviewed()}>{reviewActionBusy ? "Marking reviewed…" : "Mark all reviewed"}</button></div></WorkspaceDialog>
